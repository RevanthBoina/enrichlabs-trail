...
function generateOrbitalPositions(count: number, centerX: number, centerY: number, minRadius: number, maxRadius: number) {
  const positions = [];
...
function ConnectionLine({ from, to, progress, isActive }: { 
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      ...
      {hoveredIndex !== null && orbitalPositions[hoveredIndex] && (
        <div 
          className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-lg bg-[oklch(0.2_0.05_285)]/90 backdrop-blur-sm text-white text-sm animate-fade-in"
        >
